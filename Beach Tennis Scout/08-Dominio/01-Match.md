...
3. **O sacador é configurado em duas etapas reativas por set — nunca na criação da partida.** A partida é criada sem nenhum sacador definido (`sets[0].serverConfig: null`). No início de cada set (1º set, 2º set e Super Tie-Break, em simples e em duplas), antes do 1º ponto, um modal pergunta quem inicia sacando (`configureFirstServer`, em `packages/domain`). Em duplas, quando a dupla adversária vai sacar pela 1ª vez neste set (após o 1º game em sets regulares, após o 1º ponto no Super Tie-Break), um segundo modal pergunta qual jogador dela sacará (`configureNextServer`). Essas duas respostas alimentam `servingTeam`/`servingPlayerId` e, junto com a rotação por game/ponto do `ScoringEngine`, dispensam qualquer nova pergunta até o próximo set — que reinicia o processo do zero. Em simples só existe a primeira etapa: como não há ambiguidade de jogador, `configureFirstServer` já resolve os dois lados de uma vez. Ver [[09-ScoringEngine]] § Progressão do Saque.
